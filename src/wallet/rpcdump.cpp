...
UniValue importprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw runtime_error(
            "importprivkey \"doucoinprivkey\" ( \"label\" ) ( rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"doucoinprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
void ImportAddress(const CDoucoinAddress& address, const string& strLabel);
void ImportScript(const CScript& script, const string& strLabel, bool isRedeemScript)
{
    ...
    if (isRedeemScript) {
        ...
        ImportAddress(CDoucoinAddress(CScriptID(script)), strLabel);
    } else {
...
UniValue importaddress(const JSONRPCRequest& request)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Doucoin address or script");
    }
...
UniValue importpubkey(const JSONRPCRequest& request)
{
    ...
    ImportAddress(CDoucoinAddress(pubKey.GetID()), strLabel);
...
UniValue importwallet(const JSONRPCRequest& request)
{
    ...
    while (file.good()) {
        ...
        if (pwalletMain->HaveKey(keyid)) {
            LogPrintf("Skipping import of %s (key already present)\n", CDoucoinAddress(keyid).ToString());
            continue;
        }
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The doucoin address for the private key\n"
            "\nResult:\n"
    ...
    return CDoucoinSecret(vchSecret).ToString();
}
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by Doucoin %s\n", CLIENT_BUILD);
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        std::string strAddr = CDoucoinAddress(keyid).ToString();
        CKey key;
        if (pwalletMain->GetKey(keyid, key)) {
            file << strprintf("%s %s ", CDoucoinSecret(key).ToString(), strTime);
...
UniValue ProcessImport(const UniValue& data, const int64_t timestamp)
{
    try {
        ...
        if (!isScript) {
            address = CDoucoinAddress(output);
            if (!address.IsValid()) {
        ...
        if (isP2SH) {
            ...
            CDoucoinAddress redeemAddress = CDoucoinAddress(CScriptID(redeemScript));
            ...
            if (pubKeys.size() && keys.size() == 0) {
                ...
                CDoucoinAddress pubKeyAddress = CDoucoinAddress(pubKey.GetID());
                ...
                // Consistency check.
                if (isScript) {
                    CDoucoinAddress scriptAddress;
                    CTxDestination destination;

                    if (ExtractDestination(script, destination)) {
                        scriptAddress = CDoucoinAddress(destination);
