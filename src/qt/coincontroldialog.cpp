    ...
    model(0)
{
    ...
    // see https://github.com/doucoin/doucoin/issues/5716
...
{
    GUIUtil::setClipboard(DoucoinUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = DoucoinUnits::DUC;
    ...
    l2->setText(DoucoinUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(DoucoinUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(DoucoinUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? "~" : "") + QString::number(nBytes));        // Bytes
    l6->setText(sPriorityLabel);                                             // Priority
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(DoucoinUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    ...
    toolTip1 += tr("This means a fee of at least %1 per kB is required.").arg(DoucoinUnits::formatWithUnit(nDisplayUnit, CWallet::minTxFee.GetFeePerK())) + "<br /><br />";
    toolTip1 += tr("Can vary +/- 1 byte per input.");

    QString toolTip2 = tr("Transactions with higher priority are more likely to get included into a block.") + "<br /><br />";
    toolTip2 += tr("This label turns red, if the priority is smaller than \"medium\".") + "<br /><br />";
    toolTip2 += tr("This means a fee of at least %1 per kB is required.").arg(DoucoinUnits::formatWithUnit(nDisplayUnit, CWallet::minTxFee.GetFeePerK()));

    QString toolTip3 = tr("This label turns red, if any recipient receives an amount smaller than %1.").arg(DoucoinUnits::formatWithUnit(nDisplayUnit, ::minRelayTxFee.GetFee(546)));
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(PAIRTYPE(QString, vector<COutput>) coins, mapCoins)
    {
        ...
        BOOST_FOREACH(const COutput& out, coins.second)
        {
            ...
            {
                sAddress = QString::fromStdString(CDoucoinAddress(outputAddress).ToString());

                // if listMode or change => show doucoin address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, DoucoinUnits::format(nDisplayUnit, out.tx->vout[out.i].nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, DoucoinUnits::format(nDisplayUnit, nSum));
