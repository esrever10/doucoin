    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return DoucoinUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, DoucoinUnits::separatorNever);
            else
                return DoucoinUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != NULL) ? tr("Requested") + " ("+DoucoinUnits::name(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
