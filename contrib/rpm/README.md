...
The RPM spec file provided here is for Doucoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://doucoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Doucoin git repository](https://github.com/doucoin/doucoin) but are not part of
...
Currently only `Source10` is used, for the example `doucoin.conf` file.
...
in the doucoin git repository. At present only an SVG version of the Doucoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the doucoin
...
needed when 0.12.1 is released, a proper fix is already in the Doucoin git
master branch.
...
idea to build Doucoin Core against a static version of that library compiled
...
The `doucoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `doucoin-server` package. At this point, most RPM based
...
The `doucoin-server` package should have SELinux support. How to properly do
...
Doucoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/doucoin/). He has been
packaging Doucoin for Fedora at least since 2012.
...
4. I renamed the `doucoin` package that contains the Qt GUI to `doucoin-core` as
that appears to be how the general population refers to it, in contrast to
`doucoin-xt` or `doucoin-classic`. I wanted to make sure the general population
