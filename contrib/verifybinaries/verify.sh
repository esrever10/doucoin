...
###   This script attempts to download the signature file SHA256SUMS.asc from doucoin.org
...
WORKINGDIR="/tmp/doucoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
BASEDIR="https://doucoin.org/bin/"
VERSIONPREFIX="doucoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'doucoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Doucoin Core binary release signing key installed?"
   fi
