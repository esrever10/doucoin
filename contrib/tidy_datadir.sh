#!/bin/bash
# Copyright (c) 2013 The Doucoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Doucoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Doucoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Doucoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Doucoin 0.7 datadir."
    ;;
  3)
    echo "Detected Doucoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Doucoin 0.8 datadir."
    ;;
esac
