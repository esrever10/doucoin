Doucoin Core version 0.11.0 is now available from:

  <https://doucoin.org/bin/doucoin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer DUC/kB fee to be
...
For example, add the following to `doucoin.conf`:
...
related to the blockchain in the doucoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Doucoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Doucoin Core will 
...
For an example script see [doucoin-submittx](https://github.com/laanwj/doucoin-submittx).
...
- #5334 `cf87536` libdoucoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new doucoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only DoucoinAmountField
- #5753 `0f44672` Add doucoin logo to about screen
...
- #5809 `46bfbe7` Add doucoin-cli man page
...
- #5849 `41259ca` contrib/init/doucoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Doucoin-Qt.app to Doucoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for doucoin-tx)
