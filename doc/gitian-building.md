...
*Setup instructions for a Gitian build of Doucoin using a Debian VM or physical system.*
...
- [Building Doucoin](#building-doucoin)
...
Clone the git repositories for doucoin and Gitian and then checkout the doucoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the doucoin repository
...
To build Doucoin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md) in the doucoin repository.
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/doucoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/doucoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/doucoin-build.assert
```
...
[doucoin/gitian.sigs](https://github.com/doucoin/gitian.sigs/) repository, or if that's not possible create a pull
